...
**DuctTapeDB** is a from-scratch relational database written in Rust.
...
* **No Recursive CTEs, No Correlated Subqueries**: Common Table Expressions (CTEs) are supported in non-recursive forms only and are inline-expanded during query planning. Correlated subqueries and recursive/self-referential CTEs (`WITH RECURSIVE` or references to a CTE within its own definition) are detected and rejected at compile time.
