...
* **gRPC Server Security Constraints**: By default, the gRPC server binds to the loopback address (`127.0.0.1:50051`) without authentication or TLS. If the server is configured to bind to a non-loopback address (e.g. `0.0.0.0`), both TLS encryption (via `--tls-cert` and `--tls-key`) and shared-secret authentication (via `--auth-token`) must be configured. The server will refuse to boot if security controls are missing on external bindings.
