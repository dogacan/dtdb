//! Concurrent steady-state write target for sampling profilers.
//!
//! Mirrors the `put_random_heavy_concurrent` workload (4 threads, 200B values,
//! 256 KiB memtable so flushes/compactions are exercised) but runs in a tight loop
//! long enough for a sampler to attribute the hot path. Run:
//!
//!   cargo build -p dtdb_benchmark --example profile_write_concurrent --profile profiling
//!   samply record ./target/profiling/examples/profile_write_concurrent
...
fn main() {
    let keys = Arc::new(shuffled_indices(DISTINCT_KEYS));
    ...
    std::thread::scope(|s| {
        let chunk_size = TOTAL_PUTS / THREADS;
        for _t in 0..THREADS {
            let engine = &engine;
            let keys = &keys;
            s.spawn(move || {
